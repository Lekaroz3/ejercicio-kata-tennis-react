...
function FormTennis() {
  ...
  function onStartClick() {
    ...
    tennisGame = new TennisGame({name:player1,puntuacion:'0'},{name:player2,puntuacion:'0'})
    ...
    <div className="container">
      <div className="row">
        <div className="col-sm">
          ...
          />
          <button disabled={haGanado} style={{visibility:'hidden'}} id="btnPlayer1" className="btn btn-light row align-self-center" onClick={()=>{              
              setScore(tennisGame.wonPoint(player1))
              let ulHistorialPartida = document.getElementById('ulHistorialPartida')
              let li: any = document.createElement("li");
              li.className = "list-group-item text-dark fw-light";
              let texto: string = tennisGame.getScore()
              li.appendChild(document.createTextNode(texto))
              ulHistorialPartida?.appendChild(li)
              setHaGanado(texto.includes('Win'))
          }}>Won point</button>
        </div>
        <div className="col-sm mt-3">
           <h5>{score}</h5>
           <ul id="ulHistorialPartida" className="list-inline justify-content-center">
        ...
        <div className="col-sm">
          ...
          />
          <button disabled={haGanado} style={{visibility:'hidden'}} id="btnPlayer2" className="btn btn-light row align-self-center" onClick={()=>{              
      ...
      <div className="row align-items-center">
        <div className="col">
          <h5>{error}</h5>
          <button style={estaJugando?{display:'none'}:{display:'inline-block'}} className="btn btn-primary" onClick={onStartClick}>
            Start
          </button>
          <button style={estaJugando?{display:'inline-block'}:{display:'none'}} className="btn btn-primary" onClick={()=>{window.location.reload(true)}}>
            New Game
          </button>
        </div>
      </div>
